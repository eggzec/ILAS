C     ====================================================================
C     This file is part of ILAS - Iterative Linear Algebra Subroutines
C
C     Copyright (C) 2025  Saud Zahir
C
C     ILAS is free software: you can redistribute it and/or modify
C     it under the terms of the GNU General Public License as published by
C     the Free Software Foundation, either version 3 of the License, or
C     (at your option) any later version.
C
C     ILAS is distributed in the hope that it will be useful,
C     but WITHOUT ANY WARRANTY; without even the implied warranty of
C     MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
C     GNU General Public License for more details.
C
C     You should have received a copy of the GNU General Public License
C     along with ILAS.  If not, see <https://www.gnu.org/licenses/>.
C
C     ====================================================================
C       SGSSV   -   Gauss-Seidel Solver for A * X = B
C     ====================================================================
C       Description:
C       ------------------------------------------------------------------
C         Iterative Gauss-Seidel solver for solving linear systems of
C         equations A * X = B, where A is a square N×N matrix in
C         row-major flat array format. Single precision version.
C
...
C         A(*)      : REAL          -> flat array, row-major matrix A
C         B(N)      : REAL          -> right-hand side vector
C         X(N)      : REAL          -> input: initial guess, output: solution
C         MAX_ITER  : INTEGER       -> max number of iterations
C         TOL       : REAL          -> convergence tolerance (L∞ norm)
C         INFO      : INTEGER       -> return code:
C                                          0 = success
C                                         >0 = did not converge
C                                         <0 = illegal or zero diagonal
C     ====================================================================
      SUBROUTINE SGSSV(N, A, B, X, MAX_ITER, TOL, INFO)
